...
equations = {
    ...
    2: r" $T_i = \frac{A}{i + 1} + B$,$A = \frac{(T_0 - T_N) (N + 1)}{N}$,$B = T_0 - A$",
    3: r"$T_i = T_0 - i ^ A$,$A = \frac{\ln(T_0 - T_N)}{\ln(N)}$",
    4: r"$T_i = \frac{T_0 - T_N}{1 + e ^ {0.3(i - \frac{N}{2})}} + T_N$",
    5: r"$T_i = \frac{1}{2}(T_0 - T_N)(1 + \cos(\frac{i\pi}{N})) + T_N $",
...
def cooling_schedule_10(i: int, N: int, t: int) -> float:
    return (1 - (i / N)) ** t